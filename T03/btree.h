...
class Key {
	private:
		Student *student;
	public:
		virtal int compare(Key k);
};

class NameKey: public Key {
	public:
		int compare(Key k) {
			if (student->getName() == k.student->getName()) {
				return 0;
			} else if (student->getName() < k.student->getName()) {
				return -1;
			}
		}
...
// The main function that inserts a new key in this B-Tree
void BTree::insert(int k, Student *value)
{
	// If tree is empty
	if (root == NULL)
	{
